...
def tf_batchdot(x, y):
	...
	for i in range(x.shape[0]):
		batchdot.append(tf.tensordot(x[i], y[i], axes=1))
	return tf.convert_to_tensor(batchdot)
...
class RMSE(Metric):
	...
	def update(self, data, label, mask):
		...
		#print('label', label)
		_cnt = tf.reduce_sum(mask) * 2
		_loss = tf.reduce_sum((data - label) ** 2 * mask)
		#print('real loss', _loss)
		if self.cnt is None:
			self.cnt = 0
			self.loss = 0
		self.cnt += _cnt
...
class IndexRMSE(Metric):
	...
	def update(self, data, label, mask):
		...
		_cnt = tf_sum_exclude(mask, axis=2) * 2
